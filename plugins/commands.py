...
async def start(client, message):
    ...
    shub = await client.send_cached_media(
        chat_id=message.from_user.id,
        file_id=file_id,
        caption=f_caption,
        protect_content=True if pre == 'filep' else False,
    ...
    # Fire-and-forget: don't block handler for 20 minutes
    async def _delayed_file_cleanup():
        await asyncio.sleep(1200)
        try: await shub.delete()
        except: pass
        await asyncio.sleep(2)
        try: await dream.edit_text("<b>Thank For Using Me...</b>")
        except: pass
    asyncio.create_task(_delayed_file_cleanup())
