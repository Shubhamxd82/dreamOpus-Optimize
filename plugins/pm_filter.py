...
# Pre-compiled regex for message filtering (avoids recompilation on every message)
_IGNORE_PATTERN = re.compile(r"((^\/|^,|^!|^\.|^[\U0001F600-\U000E007F]).*)")

# Filter keyword cache with TTL to avoid querying MongoDB on every message
_FILTER_CACHE = {}  # {group_id: (keywords_list, timestamp)}
_GFILTER_CACHE = {}  # {key: (keywords_list, timestamp)}
_FILTER_CACHE_TTL = 60  # 60 seconds

async def get_cached_filters(group_id):
    """Cache filter keywords to avoid DB query on every message"""
    now = time.time()
    cached = _FILTER_CACHE.get(group_id)
    if cached and (now - cached[1]) < _FILTER_CACHE_TTL:
        return cached[0]
    keywords = await get_filters(group_id)
    keywords = list(reversed(sorted(keywords, key=len)))
    _FILTER_CACHE[group_id] = (keywords, now)
    return keywords

async def get_cached_gfilters(key):
    """Cache global filter keywords to avoid DB query on every message"""
    now = time.time()
    cached = _GFILTER_CACHE.get(key)
    if cached and (now - cached[1]) < _FILTER_CACHE_TTL:
        return cached[0]
    keywords = await get_gfilters(key)
    keywords = list(reversed(sorted(keywords, key=len)))
    _GFILTER_CACHE[key] = (keywords, now)
    return keywords

async def _schedule_delete(delay, *messages):
    """Delete messages after a delay without blocking the caller."""
...
async def next_page(bot, query):
    ...
    try:
        ...
        offset = 0
    btn_data = BUTTONS.get(key)
    if not btn_data:
        await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name),show_alert=True)
        return
    if isinstance(btn_data, dict):
        search = btn_data["search"]
        cached_total = btn_data.get("total")
    else:
        search = btn_data
        cached_total = None

    files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=offset, filter=True, cached_total=cached_total)
    try:
    ...
    if ENABLE_SHORTLINK == True:
        # Generate all shortlinks in parallel (10x faster than sequential)
        shortlinks = await asyncio.gather(
            *[get_shortlink(query.message.chat.id, f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}") for file in files]
        )
        if settings['button']:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"[{get_size(file.file_size)}] {replace_username(file.file_name)}", url=shortlink
                    ),
                ]
                for file, shortlink in zip(files, shortlinks)
            ]
        else:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"{replace_username(file.file_name)}", url=shortlink
                    ),
                    InlineKeyboardButton(
...
async def auto_filter(client, msg, spoll=False):
    ...
    reqstr = msg.from_user  # Already available — no API call needed
    if not spoll:
        message = msg
        settings = await get_settings(message.chat.id)
        if message.text.startswith("/"): return  # ignore commands
        if _IGNORE_PATTERN.findall(message.text):
            return
    ...
    if ENABLE_SHORTLINK == True:
        # Generate all shortlinks in parallel (10x faster than sequential)
        shortlinks = await asyncio.gather(
            *[get_shortlink(message.chat.id, f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}") for file in files]
        )
        if settings["button"]:
            btn = [
                [
    ...
    if offset != "":
        ...
        # Prevent memory leak: trim old BUTTONS entries when dict grows too large
        if len(BUTTONS) >= 500:
            excess = list(BUTTONS.keys())[:len(BUTTONS) - 200]
            for k in excess:
                BUTTONS.pop(k, None)
        BUTTONS[key] = {"search": search, "total": total_results}
        ...
        try:
            if message.chat.id == SUPPORT_CHAT_ID:
                ...
                if settings.get('auto_delete', True):
                    asyncio.create_task(_schedule_delete(600, hehe, message))
            ...
            if message.chat.id == SUPPORT_CHAT_ID:
                ...
                if settings.get('auto_delete', True):
                    asyncio.create_task(_schedule_delete(600, hmm, message))
        except Exception as e:
            if message.chat.id == SUPPORT_CHAT_ID:
                await message.reply_text(f"<b>Hᴇʏ {message.from_user.mention}, {str(total_results)} ʀᴇsᴜʟᴛs ᴀʀᴇ ғᴏᴜɴᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {search}. Kɪɴᴅʟʏ ᴜsᴇ ɪɴʟɪɴᴇ sᴇᴀʀᴄʜ ᴏʀ ᴍᴀᴋᴇ ᴀ ɢʀᴏᴜᴘ ᴀɴᴅ ᴀᴅᴅ ᴍᴇ ᴀs ᴀᴅᴍɪɴ ᴛᴏ ɢᴇᴛ ᴍᴏᴠɪᴇ ғɪʟᴇs. Tʜɪs ɪs ᴀ sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ sᴏ ᴛʜᴀᴛ ʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ...</b>")
            else:
                ...
                if settings.get('auto_delete', True):
                    asyncio.create_task(_schedule_delete(600, fek, message))
    else:
        if message.chat.id == SUPPORT_CHAT_ID:
            await message.reply_text(f"<b>Hᴇʏ {message.from_user.mention}, {str(total_results)} ʀᴇsᴜʟᴛs ᴀʀᴇ ғᴏᴜɴᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {search}. Kɪɴᴅʟʏ ᴜsᴇ ɪɴʟɪɴᴇ sᴇᴀʀᴄʜ ᴏʀ ᴍᴀᴋᴇ ᴀ ɢʀᴏᴜᴘ ᴀɴᴅ ᴀᴅᴅ ᴍᴇ ᴀs ᴀᴅᴍɪɴ ᴛᴏ ɢᴇᴛ ᴍᴏᴠɪᴇ ғɪʟᴇs. Tʜɪs ɪs ᴀ sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ sᴏ ᴛʜᴀᴛ ʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ...</b>")
        else:
            ...
            if settings.get('auto_delete', True):
                asyncio.create_task(_schedule_delete(600, fuk, message))
    if spoll:
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        asyncio.create_task(_schedule_delete(30, k))
        return
    movielist = []
    if not movies:
        reqst_gle = mv_rqst.replace(" ", "+")
    ...
    if settings.get('auto_delete', True):
        asyncio.create_task(_schedule_delete(600, spell_check_del))
...
async def manual_filters(client, message, text=False):
    ...
    keywords = await get_cached_filters(group_id)
    for keyword in keywords:  # Already sorted by cache
...
async def global_filters(client, message, text=False):
    ...
    name = text or message.text
    reply_id = message.reply_to_message.id if message.reply_to_message else message.id
    keywords = await get_cached_gfilters('gfilters')
    for keyword in keywords:  # Already sorted by cache
        pattern = r"( |^|[^\w])" + re.escape(keyword) + r"( |$|[^\w])"
        if re.search(pattern, name, flags=re.IGNORECASE):
